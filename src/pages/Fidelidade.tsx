              ...
              {formData.produto_tipo === "produto" ? (
                <>
                  <Select
                    ...
                    onValueChange={(v) => setFormData({ ...formData, produto_id: v, produto_size_id: "" })}
                  >
                  ...
                  {(() => {
                    ...
                    const isPizza = prod?.tipo === "pizza" && (prod?.pizzaSizes?.length || 0) > 0;
                    if (!isPizza) return null;
                    return (
                      <div>
                        <Label className="text-xs text-muted-foreground">Tamanho elegível (opcional)</Label>
                        <Select
                          value={formData.produto_size_id || "__any__"}
                          onValueChange={(v) =>
                            setFormData({ ...formData, produto_size_id: v === "__any__" ? "" : v })
                          }
                        >
                          <SelectTrigger><SelectValue /></SelectTrigger>
                          <SelectContent>
                            <SelectItem value="__any__">Qualquer tamanho</SelectItem>
              ...
              {formData.premio_tipo === "produto" && (
                <>
                  <Select
                    ...
                    onValueChange={(v) => setFormData({ ...formData, premio_produto_id: v, premio_size_id: "" })}
                  >
                    <SelectTrigger><SelectValue placeholder="Selecione o produto" /></SelectTrigger>
                    <SelectContent>
                      {menuItems.map((m) => (
